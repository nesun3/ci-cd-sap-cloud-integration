...
def Message processData(Message message) {
    ...
    if(runMode == null || runMode.trim().length() == 0 || runMode.trim() == '""')
        throw new Exception("\'Error: Value for Select either Create or Update file contents in GitHub is missing. Allowed values: create,update.\'")
    if(runMode && runMode.toString().trim().length() != 0) {
        def validValues = ["CREATE", "UPDATE"]
        if (!validValues.contains(runMode.trim().toUpperCase()))
            throw new Exception("\'Error: Value for Select either Create or Update file contents in GitHub is not valid. Allowed values: create,update.\'")
    }
    ...
    if(integrationContentType == null || integrationContentType.trim().length() == 0 || integrationContentType.trim() == '""')
        throw new Exception("\'Error: Select Artifact Type to Sync to GitHub is missing. Allowed values: all,pkg,ifl,vmp.\'")
    if(integrationContentType && integrationContentType.toString().trim().length() != 0){
        def validIntContentType = ["PKG", "IFL", "VMP", "ALL"]
        ...
        inputValues.each { value ->
            ...
            if (!validIntContentType.contains(trimmedValue))
                throw new Exception("\'Error: Value for Select Artifact Type to Sync to GitHub is not valid. Allowed values: all,pkg,ifl,vmp.\'")
        }
    }
    ...
    if(valueMappingId && valueMappingId.toString().trim().length() != 0){
        ...
        if (!inputValues.contains('ALL') && !inputValues.contains('VMP')) {
            throw new Exception("\'Error: If Value Mapping Id's exists, then Value for Select Artifact Type to Sync to GitHub must be either contain all or vmp\'")
        }
    }
